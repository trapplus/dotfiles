...
class kitty:
    def __init__(self) -> None:
        self.shell = shell
        self.kitty_is_installed: bool = (
            True if shell.run_command("kitty -v").returncode == 0 else False
        )
    ...
    def install(self) -> bool:
        if not self.kitty_is_installed:
            ...
            result = shell.run_command(
                f"curl -L {self.KITTY_INSTALLER_URL} | sh /dev/stdin"
            )
            ...
            if result.returncode == 0 and not result.stderr:
                ...
                print("Error! Installing kitty terminal from package manager...")
        ...
        if result:
            ...
            print(
                "[-] Kitty config not installed! Try manual replace kitty config directory."
            )
