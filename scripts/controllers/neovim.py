...
class neovim:
    def __init__(self) -> None:
        self.shell = shell
        self.nvim_is_installed: bool = True if shell.run_command("nvim -v").returncode == 0 else False
        self.config_is_installed: bool = False
    
    def install(self) -> bool:
        if not self.nvim_is_installed:
            print(f"Installing neovim terminal from package manager")
        ...
        if result == True:
            ...
            print("[-] neovim config not installed! Try manual replace neovim config directory.")
